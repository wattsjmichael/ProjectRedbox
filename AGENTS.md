...
Project Redbox is a Phaser 3 game built with TypeScript and Vite. Its direction combines PSO-style loot and Core progression, Vampire Survivors-style runs, and Loaded-style dark top-down sci-fi combat. Combat feel, responsive controls, and exciting loot take priority over abstract architecture.
...
- `items/`, `loot/`, `inventory/`, `core/`, `progression/`, and `persistence/` manage rewards and progression.
...
Manually test affected gameplay paths: combat responsiveness, loot collection, inventory and Core interactions, scene transitions, and reload/new-run persistence. After every task, summarize exactly which files changed and provide practical manual testing steps.
